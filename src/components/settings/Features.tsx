...
const Features = () => {
  const [features, setFeatures] = useState([
    ...
    const dropFeature = (index: number) => {
    ...
    const saveFeature = () => {
      ...
      if (featureInput && featureInput.value.trim()) {
        setFeatures([...features, featureInput.value.trim()]);
        featureInput.value = '';
      }
    };
  ...
  return (
    <div className={`rounded-lg flex flex-col items-center p-6 w-full ${localStorage.getItem('darkMode')==='true'?'bg-bgdarktheme':'bg-white'}`}>
      <div>
        <h2>{t('settingsPage.features.title')}</h2>
      </div>
      <div className="flex gap-4">
        <div className="w-[40vw] lt-sm:w-full">
          <p className=" mb-2 ml-2">{t('settingsPage.features.label')}</p>
          <input type="text" placeholder={t('settingsPage.features.placeHolder')} id="feature" className={`inputs ${localStorage.getItem('darkMode') === 'true' ? 'bg-darkthemeitems' : 'bg-white'}`} />
          ...
          <button className="btn-primary mx-auto submit mt-4 lt-sm:w-full" onClick={saveFeature}>
            {t('settingsPage.features.buttons.save')}
          </button>
        </div>
      </div>
    </div>
