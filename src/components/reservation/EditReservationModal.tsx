...
const EditReservationModal = ({
  ...
  return (
    <Portal>
      <div>
        <div className="overlay glassmorphism z-[100]" onClick={handleClose}></div>
        <div className={`sidepopup w-[40%] overflow-y-auto lt-sm:min-h-[70vh] lt-sm:max-h-[90vh] lt-sm:w-full lt-sm:bottom-0 lt-sm:overflow-y-auto h-full ${isDarkMode ? 'bg-bgdarktheme text-white' : 'bg-white'} `}>
          <CanAccess resource="reservation" action="change" fallback={
            ...
            <div className="p-4">
              ...
              <p className="text-md mb-[-.4em] font-[500]">{t('settingsPage.tags.title')}</p>
            ...
            <div className="space-y-2">
              ...
              >
                {(reservationProgressData.reserveDate === '') ? <div>{t('reservationProcess.date')}</div> : <span>{reservationProgressData.reserveDate}</span>}
                {(reservationProgressData.time === '') ? <div>{t('reservationProcess.time')}</div> : <span>{reservationProgressData.time}</span>}
                {(reservationProgressData.guests === 0) ? <div>{t('reservationProcess.guests')}</div> : <span>{reservationProgressData.guests}</span>}
              </div>
