      ...
      <div className='flex flex-col gap-4 p-2'>
        {data.map((item, index) => (
          ...
          <div className="flex flex-col items-end">
            <p className={`text-sm font-[600] ${item.status === 'Confirmed' ? 'bg-softgreentheme text-greentheme' : 'bg-softredtheme text-redtheme'} px-2 py-1 rounded-[10px]`}>{item.status}</p>
