...
const BlockReservationModal: React.FC<BlockReservationModalProps> = ({ onClose, onConfirm }) => {
  ...
  const {
    data: reservationPauses,
    isLoading: loadingReservationPauses,
    error: reservationPausesError,
    refetch: refetchReservationPauses,
  } = useList({
  ...
  const filteredTables = useCallback(() => {
    return (
      floors
        ?.flatMap((floor: any) =>
          floor.tables.map((table: any) => ({ ...table, name: `${table.name} (${floor.name})` })),
        )
        .filter((table: any) => !selectedFloor.includes(table.floor))
        .map((table: any) => ({ label: table.name, value: table.id })) || []
    )
  }, [floors, selectedFloor])

  const startBlockHours = useCallback(
    () =>
      Array.from({ length: 24 - Number(currentHour) }, (value, index) => Number(currentHour) + index).map(
        (value, i) => {
  ...
  const endBlockHours = useCallback(() => {
    return Array.from(
      { length: 24 - Number(startBlockTime?.replace(":00", "")) },
      (value, index) => Number(startBlockTime?.replace(":00", "")) + index,
    ).map((value, i) => {
      const hour = value.toString().padStart(2, "0")
      return { label: `${hour}:59`, value: `${hour}:59` }
    })
  }, [startBlockTime])
  ...
  const isValid = useCallback(() => {
    ...
    if (periodType === "TIME") if (!startBlockTime || !endBlockTime) return false
    if (blockType === "ALL") {
  ...
  const getBlockTypeLabel = (block: any) => {
    if (block.block_type === "ALL") return t("blockReservation.blockTypes.allTables", "All tables")
    if (block.block_type === "FLOOR") {
      const floorText =
        block.floor_id?.length > 1
          ? t("blockReservation.blockTypes.floors", "Floors")
          : t("blockReservation.blockTypes.floor", "Floor")
      return floorText
    }
    if (block.block_type === "TABLE") {
      const tableText =
        block.table_id?.length > 1
          ? t("blockReservation.blockTypes.tables", "Tables")
          : t("blockReservation.blockTypes.table", "Table")
      return tableText
    }
  ...
  const getReservationChannel = (block: any) => {
    if (block.block_online && block.block_backoffice)
      return t("blockReservation.channels.onlineAndBackOffice", "Online & Back-Office")
    if (block.block_online) return t("blockReservation.channels.onlineOnly", "Online only")
    if (block.block_backoffice) return t("blockReservation.channels.backOfficeOnly", "Back-Office only")
    return ""
  }
  ...
  const DateSelectionModal: React.FC<DateSelectionModalProps> = ({ focusedDate, setFocusedDate, handleDateClick }) => {
