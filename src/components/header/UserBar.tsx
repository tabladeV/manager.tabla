...
const UserBar = () => {
  ...
  const setLanguage = (language: string | undefined) => {
    if (language) {
      ...
      // REMOVED: i18n.changeLanguage(language) - let RootLayout handle this
      setShowLang(false)
    }
  }
  ...
  const getCurrentLanguageIcon = () => {
    ...
    if (preferredLanguage === "es") return spanish
    ...
    <div className="relative">
      ...
      {showLang && (
        ...
        >
          ...
          >
            <img src={english || "/placeholder.svg"} className="w-6 h-6" alt="English" />
            <span>English</span>
          </button>
          <button
            className={`flex items-center gap-2 p-2 rounded hover:bg-opacity-80 hover:bg-gray-100 dark:hover:bg-gray-700 ${
          ...
          >
            <img src={french || "/placeholder.svg"} className="w-6 h-6" alt="French" />
            <span>Français</span>
          </button>
          <button
            className={`flex items-center gap-2 p-2 rounded hover:bg-opacity-80 hover:bg-gray-100 dark:hover:bg-gray-700 ${
          ...
          >
            <img src={arabic || "/placeholder.svg"} className="w-6 h-6" alt="Arabic" />
            <span>العربية</span>
          </button>
          {/* Fixed Spanish button - was using 'ar' condition instead of 'es' */}
          <button
            className={`flex items-center gap-2 p-2 rounded hover:bg-opacity-80 hover:bg-gray-100 dark:hover:bg-gray-700 ${
              preferredLanguage === "es" ? "bg-gray-100 dark:bg-gray-700" : ""
            }`}
            onClick={() => setLanguage("es")}
          >
            <img src={spanish || "/placeholder.svg"} className="w-6 h-6" alt="Spanish" />
            <span>Español</span>
          </button>
        </div>
      )}
      ...
      <div className="flex items-center gap-2">
        ...
        >
          <img src={getCurrentLanguageIcon() || "/placeholder.svg"} className="w-6 h-6" alt="Language" />
        </button>
        ...
        >
          {userData?.bo_profile?.image ? (
            <img
              ...
              src={userData.bo_profile.image || "/placeholder.svg"}
              alt={userFullName}
            />
          ) : (
      ...
      {showProfile && (
        <div ref={profileMenuRef} className={`absolute right-0 mt-2 w-64 rounded-md shadow-lg z-50 ${bgColor} py-1`}>
          {/* User Info Section */}
          ...
          >
            ...
            <span>{t("header.buttons.profile")}</span>
          </Link>
          ...
          >
            {isDarkMode ? (
              <>
                ...
                <span>{t("header.buttons.lightMode")}</span>
              </>
            ) : (
              <>
                <Moon size={18} className="mr-2" />
                <span>{t("header.buttons.darkMode")}</span>
              </>
            )}
          </button>
          ...
          >
            {isFullScreen ? (
              <>
                ...
                <span>{t("header.buttons.exitFullscreen")}</span>
                {showShortCut && (
                  <span className="ml-3 flex items-center gap-1 lt-sm:hidden">
                    <span className="text-[10px] text-white/80 py-[0px] px-2 btn rounded-md">Ctrl</span>
                    <span className="text-[10px] text-white/80 py-[0px] px-2 btn rounded-md">M</span>
                  </span>
                )}
              </>
            ) : (
              <>
                ...
                <span>{t("header.buttons.enterFullscreen")}</span>
                {showShortCut && (
                  <span className="ml-3 flex items-center gap-1 lt-sm:hidden">
                    <span className="text-[10px] text-white/80 py-[0px] px-2 btn rounded-md">Ctrl</span>
                    <span className="text-[10px] text-white/80 py-[0px] px-2 btn rounded-md">M</span>
          ...
          >
            ...
            <span>{t("header.buttons.logout")}</span>
          </button>
        </div>
      )}
    </div>
